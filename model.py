...
class CrystalGraphConvNet(nn.Module):
    def __init__(self, orig_atom_fea_len, nbr_fea_len,
        ...
        self.extra_feat_net = nn.Sequential(
            nn.Linear(n_extra_features, h_fea_len // 4),  # 将原始额外特征（n_extra_features维度，如 19）线性压缩到h_fea_len//4维度（例如h_fea_len=256时，压缩到 64 维），减少计算量并统一特征尺度。
        ...
        self.conv_to_fc = nn.Linear(atom_fea_len + (h_fea_len // 4), h_fea_len)#将 “晶体结构特征” 与 “处理后的额外特征” 融合，并转换到全连接层的输入维度将1和2拼接后的维度映射为3
        ...
        if n_h > 1:
            self.fcs = nn.ModuleList([nn.Linear(h_fea_len, h_fea_len)
                                      for _ in range(n_h-1)])#创建 n_h-1 个维度不变的全连接隐藏层，用于对融合后的特征进行深度加工，通过多次线性变换和非线性激活，捕捉更复杂的特征模式，
    ...
    def forward(self, atom_fea, nbr_fea, nbr_fea_idx, crystal_atom_idx, extra_fea):
        ...
        combined_fea = torch.cat([crys_fea, extra_fea], dim=1)#将crys_fea, extra_fea在列方向dim=1拼接，拼接后的维度为atom_fea_len + h_fea_len//4
