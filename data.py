...
class CIFData(Dataset):
    def __init__(self, root_dir, feature_file,max_num_nbr=12, radius=8, dmin=0, step=0.2,
        ...
        # ---------- 1. 读 CSV：只取 material_id 与 target 两列 ----------
        id_prop_file = os.path.join(root_dir, 'id_prop.csv')
        assert os.path.exists(id_prop_file), 'id_prop.csv does not exist!'

        with open(id_prop_file, newline='') as f:
            reader = csv.DictReader(f)          # 用 DictReader 按列名拿
            if 'material_id' not in reader.fieldnames:
                raise KeyError('CSV 缺少列名: material_id')
            if 'target' not in reader.fieldnames:
                raise KeyError('CSV 缺少列名: target')

            self.id_prop_data = []
            for row in reader:
                mid = row['material_id'].strip()
                tgt = float(row['target'])      # 若空值请自行补异常处理
                self.id_prop_data.append((mid, tgt))
        ...
        assert os.path.exists(feature_file), 'Feature file does not exist!'#feature_file的读取
        self._load_extra_features(feature_file)#调用下方类
        self._compute_feature_normalization()#调用下方类
